...
io.on("connection", (socket) => {
    ...
    socket.on("create new question", (options, question_id, question) => {
        Question_1.Question.instances.push(new Question_1.Question(question_id, options, question, socket.id, socket));
    });
    ...
    socket.on("user response", (option_index, question_id) => {
        ...
        (0, Question_1.sendAnalyticsToCreator)(question_id);
    });
    // For server debugging
    socket.on("send broadcast", () => {
        socket.emit("questions", Question_1.Question.instances.map((ele) => {
            return {
    ...
    socket.on("get question", (question_id) => {
        socket.emit("get question", Question_1.Question.instances
            .map((ele) => {
            return {
                question_id: ele.question_id,
                question: ele.question,
